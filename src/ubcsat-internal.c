...
const char *sValidStatCodes[NUMVALIDSTATCODES] = {"all", "mean", "stddev", "cv", "var", "stderr", "vmr", "sum",
                                                  "median", "min", "max", "q05", "q10", "q25", "q75", "q90", "q95",
                                                  "q98", "qr75/25", "qr90/10", "qr95/05"};
...
ALGPARM *AddParmCommon(ALGPARMLIST *pParmList, const char *sSwitch, const char *sTerseDescription,
                       const char *sVerboseDescription, const char *sTriggers);
...
REPORT *CreateReport(const char *sID, const char *sDescription, const char *sVerboseDescription,
                     const char *sOutputFile, const char *sTriggers) {
  REPORT *pRep;
...
void CreateTrigger(const char *sID, enum EVENTPOINT eEventPoint, FXNPTR pProcedure, char *sDependencyList,
